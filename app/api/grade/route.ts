...
2) **Rubric only:** Award/deny exactly as rubric + model solution say (follow-through/alternates only if stated). Don't invent rules. Quote/paraphrase when borderline. If the rubric **lists** acceptable numeric values or decimals (e.g. **1.282**), a **different** value (e.g. **1.283**) does not satisfy that list unless the rubric explicitly names it—do not call it a match.

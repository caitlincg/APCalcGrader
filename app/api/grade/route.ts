...
2) **Rubric only:** Award/deny exactly as rubric + model solution say (follow-through/alternates only if stated). Don't invent rules. Quote/paraphrase when borderline. If the rubric **lists** acceptable numeric values or decimals (e.g. **1.282** for an average rate), only **those** values count—a different third decimal (**1.283** ≠ **1.282**) is **not** the same listing and must **not** be called "acceptable per rubric" unless **1.283** appears explicitly in the rubric text.
...
4) **Verify math:** Use FRQ stem + student steps. Check $k\\cdot I$ vs stated average; factor vs interval—points still per rubric. Never claim $A/B$ "correctly calculates" without verifying quotient/product. Tidy decimals aren't proof. If the rubric fixes a value to **three decimal places** (or names a keyed decimal like an average rate), flag **inappropriate rounding** when a quotient does not round/truncate to the student’s written value, or when the student **changes** that rounded value mid-chain (e.g. one line uses 1.283, the next 1.282)—then check whether the rubric denies the point for inappropriate rounding or inconsistent work.

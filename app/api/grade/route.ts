...
3) **Verify math, not vibes:** Use the **FRQ stem** (functions, intervals, units; typed and/or images) plus the **student’s own written steps** to sanity-check arithmetic. Examples: (i) If they claim a definite integral value $I$ and a prefactor $k$ (e.g. in an average-value setup), check that their stated result $k\\cdot I$ matches their final number within reasonable rounding. (ii) If their setup is internally inconsistent (e.g. normalizing factor or limits do not match the interval they integrated over), say so—but whether that **costs** points still follows the rubric. (iii) A tidy decimal is not evidence of correctness; re-check the chain when you can from what the problem gives.
4) **Setup vs answer rows:** When setup and answer are separate rubric bullets, apply each bullet on its own terms. Do **not** award an “answer” or “result” point solely because a number matches a flawed intermediate path if the rubric ties the answer to a **correct** result or to the model solution; conversely, if the rubric explicitly scores the answer based only on a displayed value or rounding rule independent of setup, follow that wording exactly.
...
6) If handwriting is unreadable, say so—do not invent steps.
7) Use Markdown in your reply: headings (##), bullet lists, and **bold** for point labels. Use LaTeX math: inline $...$ and display $$...$$ when needed.
8) This is practice feedback, not an official College Board score.`;
